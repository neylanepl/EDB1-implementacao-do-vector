...
void TestManager::result(const std::string& key, bool value, int line) {
    ...
    if (old_entry.m_result == Entry::result_t::SUCCESS or old_entry.m_result == Entry::result_t::UNDEFINED) {
...
void TestManager::summary(void) const {
    ...
    std::sort(sorted_list.begin(), sorted_list.end(),
              [](const hash_item& h1, const hash_item& h2) -> bool { return h1.second.m_seq < h2.second.m_seq; });
    ...
    for (const auto& t : sorted_list) {
        ...
        if (not t.second.m_enabled)
            n_disabled++;
        else if (t.second.m_result == TestManager::Entry::result_t::SUCCESS)
            n_successful++;
        else if (t.second.m_result == TestManager::Entry::result_t::FAILED)
            n_failed++;
        else if (t.second.m_result == TestManager::Entry::result_t::UNDEFINED)
            n_undefined++;
    }
    ...
    if (n_disabled != 0)
        std::cout << "[ "
                  << "\e[1;36mDISABLED\e[0m"
                  << "  ] " << n_disabled << " tests.\n";
    if (n_undefined != 0)
        std::cout << "[ "
                  << "\e[1;35mUNDEFINED\e[0m"
                  << " ] " << n_undefined << " tests.\n";
}
