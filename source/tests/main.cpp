...
int main(void) {
...

    {
        ...
        for (auto i{0u}; i < vec.size(); ++i) EXPECT_EQ((int)i + 1, vec[i]);
    }

    {
        ...
        for (auto i{0u}; i < vec3.size(); ++i) EXPECT_EQ(vec[i + offset], vec3[i]);
    }
...

    {
        ...
        for (auto i{4u}; i >= vec.size(); --i) EXPECT_EQ((int)i + 1, vec[i]);
...

    {
        ...
        // Verify the elements.
        for (auto i{0u}; i < vec.size(); ++i) EXPECT_EQ(value, vec[i]);
...

    {
        ...
        for (auto i{0u}; i < vec.size(); ++i) EXPECT_EQ(vec[i], vec2[i]);
    }
...

    {
        ...
        for (auto i{0u}; i < vec.size(); ++i) EXPECT_EQ(vec.at(i), vec2.at(i));
...

    {
        ...
        for (auto i{0u}; i < vec.size(); ++i) vec.at(i) = vec2.at(i);
        for (auto i{0u}; i < vec.size(); ++i) EXPECT_EQ(vec.at(i), vec2.at(i));
...

    {
        ...
        auto i{0};
        for (const auto& e : vec) EXPECT_EQ(e, ++i);
    }
...

    {
        ...
        auto it = vec.begin();
        int i{1};
        while (it != vec.end()) EXPECT_EQ(*it++, i++);
    }
...

    {
        ...
        auto it1 = vec.begin();
        auto it2 = vec.begin();
        while (it1 != vec.end()) EXPECT_EQ(it1++, it2++);
    }
